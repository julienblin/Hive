...
namespace Hive.Tests.Meta.Impl
{
	...
	{
		[Theory]
		[MemberData(nameof(GetModelOkData))]
		public async Task GetModelOk(ModelData modelData, Action<IModel, ModelData> asserts)
		{
			var metaService = new MetaService(
				new ConfigServiceMock(),
				new MetaRepositoryMock(modelData),
				new ValueTypeFactory(), 
				new ModelCacheMock()
			);

			var model = await metaService.GetModel(modelData.Name, CancellationToken.None);

			asserts(model, modelData);
		}

		public static IEnumerable<object[]> GetModelOkData()
		{
